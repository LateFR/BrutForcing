...
def hacher_tentative(tentative):
    """Utilise SHA-256 pour hacher la tentative"""
    return hashlib.sha256(tentative.encode()).hexdigest()
...
def brute_force(mot_de_passe_a_trouver, longueur_max, intervalle):
    ...
    essais_1 = essais
    tentatives_deja_testees = set()  # Utilisation d'un set pour stocker les tentatives hachées
    ...
    for longueur in range(1, longueur_max + 1):
        for combinaison in itertools.product(caracteres, repeat=longueur):
            ...
            tentative_hachee = hacher_tentative(tentative)
            
            if tentative_hachee in tentatives_deja_testees:
                continue

            tentatives_deja_testees.add(tentative_hachee)
...
try:
    ...
    print(f"en {elapsed_time:,.0f} secondes soit {elapsed_time_en_minutes:,.1f} minutes soit {elapsed_time/3600:,.1f} heures "
          f"soit {elapsed_time/86400:,.1f} jours soit {elapsed_time/31536000:,.1f} ans.")
...
if mot_de_passe_trouve:
    ...
    print(f"Mot de passe non trouvé après {nombre_essais:,} essais.")
